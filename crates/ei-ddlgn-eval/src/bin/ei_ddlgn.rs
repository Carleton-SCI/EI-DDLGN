...
/// Boolean encrypted inference with runtime public-constant propagation.
///
/// This binary uses the same model and test data format as `lgn_eval_boolean`.
/// It keeps input wires encrypted, but when a gate output is known to be public
/// from its truth table and current public inputs, later gates reuse that public
/// value instead of evaluating a two-input encrypted gate.
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Step 2/4: Generating TFHE Boolean keys...");
    let keygen_start = Instant::now();
    let engine = EiDdlgnEngine::new_with_param(DEFAULT_PARAMETERS_KS_PBS);
    let compiled_layers = engine.compile_layers(&layers);
    println!(
        "  Key generation done in {:.3}s",
    ...
    println!("Step 4/4: Starting EI_DDLGN encrypted evaluation...");
    ...
    for (idx, result) in rdr.records().enumerate() {
        ...
        let out = engine.eval_compiled_layers(&compiled_layers, &enc_input, USE_PARALLEL);
...
fn parse_args() -> Result<(PathBuf, Option<usize>), Box<dyn Error>> {
    ...
    let base = match args.next() {
        ...
        None => {
            eprintln!("Usage: EI_DDLGN <model_dir> [--limit N]");
            std::process::exit(1);
        }
    };
...
fn print_network_summary(meta: &Meta, layers: &[Vec<(usize, usize, lgn_eval::model::Gate)>]) {
    ...
    let total_neurons: usize = layers.iter().map(|layer| layer.len()).sum();
