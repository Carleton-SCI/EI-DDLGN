...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.verbose {
        ...
        eprintln!("Generating TFHE Boolean keys once for the whole EI_DDLGN sweep...");
    }
    let keygen_start = Instant::now();
    let engine = EiDdlgnEngine::new_with_param(DEFAULT_PARAMETERS_KS_PBS);
    if args.verbose {
        eprintln!(
            "Key generation completed in {:.3}s.",
    ...
    let output_path = args
        .output
        .unwrap_or_else(|| args.root.join("ei_ddlgn_sweep_timing.txt"));
...
fn parse_args() -> Result<Args, Box<dyn Error>> {
    ...
    let root = match values.next() {
        ...
        None => {
            eprintln!(
                "Usage: ei_ddlgn_sweep <MODEL_SWEEP_DIR> [--limit N] [--output PATH] [--verbose]"
            );
            std::process::exit(1);
        }
    };
...
fn evaluate_model(
    ...
    for (idx, result) in rdr.records().enumerate() {
        ...
        let _out = engine.eval_compiled_layers(&compiled_layers, &enc_input, USE_PARALLEL);
    ...
    if verbose {
        eprintln!(
            "Finished depth={} width={} avg_eval={:.6}s/vector",
            depth,
            width,
...
fn format_timing_table(rows: &[ModelTiming]) -> Result<String, Box<dyn Error>> {
    ...
    table.push_str("EI_DDLGN Boolean inference time, avg eval seconds/vector\n");
