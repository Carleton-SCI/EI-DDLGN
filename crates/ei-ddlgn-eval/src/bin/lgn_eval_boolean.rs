...
/// Main entry point for Boolean evaluation.
///
/// Steps:
/// 1. Parse CLI args.
/// 2. Load model.
/// 3. Generate TFHE Boolean keys.
/// 4. Evaluate network on encrypted data (boolean gates).
...
fn parse_args() -> Result<(PathBuf, Option<usize>), Box<dyn Error>> {
    ...
    let base = match args.next() {
        ...
        None => {
            eprintln!("Usage: lgn_eval_boolean <model_dir> [--limit N]");
            std::process::exit(1);
        }
    };
...
/// Decodes final layer (summing votes).
