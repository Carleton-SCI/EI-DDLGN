...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.verbose {
        ...
        eprintln!("Generating TFHE Boolean keys once for the whole sweep...");
    }
    let keygen_start = Instant::now();
    let engine = BooleanEngine::new_with_param(DEFAULT_PARAMETERS_KS_PBS);
    if args.verbose {
        eprintln!(
    ...
    let output_path = args
        .output
        .unwrap_or_else(|| args.root.join("boolean_sweep_timing.txt"));
...
fn parse_args() -> Result<Args, Box<dyn Error>> {
    ...
    let root = match values.next() {
        ...
        None => {
            eprintln!(
                "Usage: lgn_eval_boolean_sweep <MODEL_SWEEP_DIR> [--limit N] [--output PATH]"
            );
            std::process::exit(1);
        }
    };
...
fn evaluate_model(
    ...
    for (idx, result) in rdr.records().enumerate() {
        ...
        let _out = engine.eval_layers(&layers, &enc_input, USE_PARALLEL);
...
fn format_timing_table(rows: &[ModelTiming]) -> Result<String, Box<dyn Error>> {
    ...
    table.push_str("Encrypted Boolean inference time, avg eval seconds/vector\n");
