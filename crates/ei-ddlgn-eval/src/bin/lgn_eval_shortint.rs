...
use lgn_eval::shortint_backend::ShortintEngine;
use std::error::Error;
...
/// Main entry point for Shortint evaluation.
///
/// Steps:
/// 1. Parse CLI args (model dir, optional limit).
/// 2. Load model structure and metadata.
/// 3. Generate TFHE Shortint keys and Look-Up Tables.
/// 4. Iterate over the test dataset, encrypt inputs, evaluate, and verify results.
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Step 2/4: Generating TFHE Shortint keys + LUTs...");
    // Using V1_1 parameters for speed/security balance suitable for this demo.
    let keygen_start = Instant::now();
    let engine = ShortintEngine::new_with_param(
        parameters::v1_1::V1_1_PARAM_MESSAGE_1_CARRY_1_KS_PBS_GAUSSIAN_2M64,
    );
    println!(
        "  Key+LUT generation done in {:.3}s",
        keygen_start.elapsed().as_secs_f64()
    );
    ...
    for (idx, result) in rdr.records().enumerate() {
        ...
        // Remaining columns are input features (0 or 1).
...
/// Expects a directory path and an optional `--limit N` flag.
fn parse_args() -> Result<(PathBuf, Option<usize>), Box<dyn Error>> {
    let mut args = std::env::args().skip(1);
    let base = match args.next() {
        Some(value) => PathBuf::from(value),
        None => {
            eprintln!("Usage: lgn_eval_shortint <model_dir> [--limit N]");
            std::process::exit(1);
        }
    };
...
/// Decodes the final layer output into a class prediction.
///
/// The final layer usually contains `class_count * votes_per_class` bits.
/// We count how many '1's are active for each class chunk and pick the class with the most votes.
...
fn report_stats(
    ...
    if total > 0 {
        ...
        println!("Avg encrypt/vector: {:.6}s", avg_encrypt);
        println!("Avg eval/vector: {:.6}s", avg_eval);
