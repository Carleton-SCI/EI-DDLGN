...
use crate::core_crypto::entities::GlweSecretKeyView;
...
//Added for GLWE secret key extraction (by Mahmoud)
/// Returns a view of the GLWE secret key used for encrypting the TLWE
    pub fn glwe_secret_key(&self) -> GlweSecretKeyView<'_, u64> {
        match &self.atomic_pattern {
            AtomicPatternClientKey::Standard(inner) => inner.glwe_secret_key.as_view(),
            _ => panic!("GLWE secret key extraction only supported for Standard atomic pattern"),
        }
    }
}
